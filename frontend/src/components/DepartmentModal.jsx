...
const DepartmentModal = ({ isOpen, onClose, onSuccess, department = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/departments/${department.id}`, { name: trimmedName });
      } else {
        await api.post('/departments', { name: trimmedName });
      }
      onSuccess();
      onClose();
    } catch (err) {
      console.error('Department operation failed', err);
      setError(err.response?.data?.detail || 'An unexpected error occurred while saving the department.');
    } finally {
