...
const Assets = () => {
  ...
  // RBAC checks: Admin + Technician can Create & Edit; Admin only can Delete
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const [assetsRes, catsRes, deptsRes, secsRes] = await Promise.all([
        api.get('/assets'),
      ...
      setError(
        ...
        'Failed to load assets data. Please check your network connection or verify your session.'
      );
    } finally {
      setLoading(false);
    }
  ...
  // Lookup maps for foreign key IDs -> readable names
  ...
  const handleDelete = async (asset) => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to delete asset "${asset.asset_tag} - ${asset.name}"?`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setActionError(
        err.response?.data?.detail ||
        'Failed to delete asset. An unexpected error occurred.'
      );
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-gray-500 max-w-3xl">
            Manage plant physical equipment, tracking tags, locations, maintenance statuses, and financial metadata.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="border border-dashed border-gray-200 rounded-xl m-8 p-12 text-center text-gray-400">
            ...
            <span className="block text-sm font-semibold text-gray-700">No assets found</span>
            <p className="mt-1 text-xs text-gray-400">
              {canManage
                ? 'Click "Add Asset" above to register the first equipment unit.'
                : 'No assets currently recorded in the database.'}
            </p>
          </div>
        ) : (
                  ...
                  <tr key={asset.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {categoryMap[asset.category_id] || (
                        <span className="text-gray-400 italic">ID: {asset.category_id}</span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {departmentMap[asset.department_id] || (
                        <span className="text-gray-400 italic">ID: {asset.department_id}</span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {sectionMap[asset.section_id] || (
                        <span className="text-gray-400 italic">ID: {asset.section_id}</span>
                      )}
                    </td>
